...
2. 在下表**追加一行**，`NNN = 当前最大号 + 1`，status 填 `reserved`：
   ```
   | 005 | inventory-alert | 甲 | 2026-06-10 | reserved | 库存预警 |
   ```
3. 只提交这一行并 push 进 main（仅这一行，可免 PR）：
   ```
   git add docs/engineering/registry.md
   git commit -m "chore(registry): 预留 NNN=005 inventory-alert"
   git push
   ```
   - 并发抢同号时，后 push 者会 non-fast-forward 被拒、或 PR 合并时本表冲突 → **当场暴露** → 改取下一个号重来。
4. 再开 `feat/005-inventory-alert` 分支，跑 `/spec-prd`，直接用 005。
...
| --- | --- | --- | --- | --- | --- |
| 001 | exam-assistant-booking | —（补填） | 2026-05-29 | reserved | 考试助理预约 |
| 002 | exam-open-status-mock-retake | —（补填） | 2026-05-29 | reserved | 考试开放状态/模拟重考 |
| 003 | admin-reset-user-password | —（补填） | 2026-06-03 | reserved | 管理员重置用户密码 |
| 004 | composite-question-indefinite-choice | —（补填） | 2026-06-04 | reserved | 复合题/不定项选择 |
